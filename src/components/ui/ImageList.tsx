  ...
  return (
    <div className="flex flex-wrap gap-4 mt-6 max-h-[70vh] overflow-y-auto p-8">
      {selectedImages.map((image, index) => (
        <div
          key={image.name}
          className={`relative transition-transform ease-in-out duration-500 px-[4px] ${hoveredImage === index ? "z-10" : "z-0"} ${hoveredImage === index ? "scale-105" : "scale-100"
            }`}
        ...
        >
          <img
            ...
            alt={image.name}
            className="w-44 h-44 rounded-2xl shadow-lg"
          />
          <div
