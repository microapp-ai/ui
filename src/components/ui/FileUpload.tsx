  ...
  return (
    <div className={cn("flex flex-col",className )}style={{ width }}>
      ...
      <div
        {...getRootProps()}
        className={cn(
          "border-2 border-dashed p-[16px] rounded-lg transition-colors duration-300 flex flex-col items-center justify-center gap-4 relative",
          "hover:border-ring hover:bg-muted",
          {
            "border-actionable-secondary-hover": !isDragActive && !error,
            "border-ring border-solid": isDragActive,
          ...
          <ul className="mt-4 space-y-2 w-full">
            {files.map((file, index) => (
              <li key={index} className="flex justify-between items-center text-white">
                ...
                <button onClick={() => clearFile(file.name)} className="text-red-500">
                  Remove
                </button>
              </li>
            ))}
