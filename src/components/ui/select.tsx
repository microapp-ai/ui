...
const SelectTrigger = React.forwardRef<
  ...
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between rounded-md border border-foreground-border bg-background text-foreground placeholder:text-muted-foreground focus:outline-none focus:bg-surface-activeActionableSecondary disabled:cursor-not-allowed disabled:opacity-50 disabled:bg-surface-muted",
      sizeStyles[size],
  ...
  >
    ...
    <SelectPrimitive.Icon asChild>
        <ChevronDown className="-4 w-4 opacity-50 text-muted-foreground" />
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-md bg-popover rounded-sm text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
        position === "popper" &&
...
>(({ className, children, item_indicator_icon, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-surface-hoverActionableSecondary focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
    {...props}
  >
  ...
  return (
    <div className={`select-container ${className}`} style={{ width }}>
      ...
      <Select onOpenChange={(open) => { }} disabled={disabled}>
        ...
        <SelectContent  className={`w-full ${className}`}> {/* Pass the width */}
          {options.map((option) => (
